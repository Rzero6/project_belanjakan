...
class RegisterBloc extends Bloc<RegisterEvent, RegisterState> {
  ...
  void _onFormSubmitted(FormSubmitted event, Emitter<RegisterState> emit) async {
    emit(state.copyWith(formSubmissionState: FormSubmitting()));
    try {
      User userData = await registerRepository.register(event.username, event.password, event.noTlp, event.tanggalLahir, event.email);
